...
unsigned int* DF(NodLP* pLP, unsigned int start, unsigned int nrVarfuri) {
	...
	while (stack) {
		...
		NodLS* tmpLS = tmpLP->primListaVecini; // adresa de inceput a listei de vecini pt varf
		while (tmpLS)
		{
			if (flags[tmpLS->idNodAdiacent - 1] == 0)
			{
				...
				flags[tmpLS->idNodAdiacent - 1] = 1; // marchez tmpLS->idNodAdiacent ca fiind vizitat
			}
