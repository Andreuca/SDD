	...
	if (p->stud.id == idStud1 || p->stud.id == idStud2) {
		...
		while (q->next) {
			...
			if (q->stud.id == idStud2 || q->stud.id == idStud1) {
				// daca nodurile sunt adiacente
				if (p->next == q) {
					return interschimbAdiacente(p, p->stud.id);
				}
				else {
					q->next = p->next;
					prevQ->next = p;
	...
	while (vb == 1) {
		...
		while (temp->next) { // traversare lista pana pe penultimul nod
			...
			if (temp->stud.id > n_temp->stud.id) {
				...
				vb = 1;
				lst = interschimbAdiacente(lst, temp->stud.id); // temp = temp -> next rezulta din operatia din interschimb
			}
			else {
				temp = temp->next;
			}
...
void main() {
	...
	//prim = interschimbAdiacente(prim, 129);
	//printf("\nTraversare lista simpla dupa interschimb adiacente:\n");
	//temp = prim;
	//while (temp != NULL) {
	//	printf("Student id: %d, %s, %s", temp->stud.id, temp->stud.nume, temp->stud.nrGrupa);
	...
	// Apel interschimb noduri oarecare
	prim = interschimbNoduriOarecare(prim, 99, 129);
	printf("\nTraversare lista simpla dupa interschimb noduri oarecare:\n");
	temp = prim;
	while (temp != NULL) {
		printf("Student id: %d, %s, %s", temp->stud.id, temp->stud.nume, temp->stud.nrGrupa);
	...
	// Apel sortare oarecare
	prim = sortareOarecare(prim);
	printf("\nLista sortata apeland interschimbare noduri oarecare\n");
	temp = prim;
	while (temp != NULL) {
		printf("Student id: %d, %s, %s", temp->stud.id, temp->stud.nume, temp->stud.nrGrupa);
	...
	//printf("\n\nTraversare lista simpla dupa sortare bubble sort:\n");
	//temp = prim;
	//while (temp != NULL) {
	//	printf("Student id: %d, %s, %s", temp->stud.id, temp->stud.nume, temp->stud.nrGrupa);
